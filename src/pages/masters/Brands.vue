...
const { items, loading, error, fetchData, createItem, updateItem, deleteItem } = useCrud(
  ...
  apiService.createBrand,
  apiService.updateBrand,
  apiService.deleteBrand,
)
...
async function handleDelete(id: string) {
  if (confirm('Hapus brand ini?')) await deleteItem(id)
}
</script>
  ...
  <div>
    <div class="flex items-center justify-between mb-4">
      <div class="relative w-64">
        ...
        <Input v-model="searchQuery" placeholder="Cari brand..." class="pl-9" />
      </div>
    ...
    <Teleport to="body">
      <div v-if="dialog.open.value" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="dialog.close()">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl mx-4">
          <h2 class="text-lg font-semibold mb-4">{{ dialog.editing.value ? 'Edit Brand' : 'Tambah Brand' }}</h2>
