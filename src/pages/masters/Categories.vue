...
const dialog = useFormDialog({ id: '', name: '', description: '' })
...
async function handleSave() {
  ...
  try {
    if (dialog.editing.value) {
      await updateItem(dialog.form.id, { name: dialog.form.name, description: dialog.form.description })
    } else {
      await createItem({ name: dialog.form.name, description: dialog.form.description })
    }
    dialog.close()
  } finally {
        ...
        <tbody>
          <tr v-for="item in filteredItems" :key="item.id" class="border-b last:border-0 hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right">
              <Button variant="ghost" size="icon" @click="dialog.openEdit(item)">
                <Pencil class="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" class="text-red-500" @click="handleDelete(item.id)">
    ...
    <Teleport to="body">
      <div v-if="dialog.open.value" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="dialog.close()">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl mx-4">
          <h2 class="text-lg font-semibold mb-4">{{ dialog.editing.value ? 'Edit Kategori' : 'Tambah Kategori' }}</h2>
          <form @submit.prevent="handleSave" class="space-y-4">
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              <Button type="submit" :disabled="dialog.submitting.value">
                <Loader2 v-if="dialog.submitting.value" class="mr-2 h-4 w-4 animate-spin" />
                Simpan
              </Button>
            </div>
          </form>
